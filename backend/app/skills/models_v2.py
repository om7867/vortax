...
class UserSkillProfile(Base):
    ...
    user = relationship("User", backref="user_skills_list")
    skill = relationship("SkillMaster", back_populates="user_skill_profiles")
    
    __table_args__ = (
        UniqueConstraint('user_id', 'skill_id', name='unique_user_skill_pair'),
        CheckConstraint('self_rating >= 1 AND self_rating <= 5', name='check_rating_range_v2'),
    )
